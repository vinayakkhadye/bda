...
document.addEventListener("contextmenu", function(e) {
    ...
    FB.getLoginStatus(function(e) {
        "connected" === e.status ? (e.authResponse.userID, e.authResponse.accessToken, fb_api()) : ("not_authorized" === e.status, fb_login())
    })
}), $(function() {
    "use strict";
    var e = !1,
    ...
    $("#speciality").autocomplete({
        ...
        minChars: 3,
        serviceUrl: BASE_URL+"api/search/doctor_web/" + cityName,
        onSelect: function(e) {
        ...
        minChars: 3,
        serviceUrl: BASE_URL+"api/search/clinic_web/" + cityName,
        onSelect: function(e) {
    ...
    $("#term").length > 0 && $(".owl-carousel .owl-item > .item").live("click", function() {
        ...
        var e = $("#speciality").attr("url-data"),
            t = ($("#location").attr("url-data") && $("#location").attr("url-data")=='all')?'':$("#location").attr("url-data");
        ...
        $(e).toggle("fast", "linear", function() {
            0 == $(e).children(".phone-no-panel").length && $.ajax({
                type: "POST",
                url: BASE_URL+"api/search/phone_number",
                dataType: "json",
                data: {
                    clinic_id: i,
                    doctor_id: t,
										no_doctor_details:true
                },
                success: function(t) {
                    if (1 == t.response.status) {
												
                        var i = '' + (t.response.knowlarity.number?'<div class="phone-no-panel"><p class="phone-no">'+t.response.knowlarity.number+'</p>':'') + ''+(t.response.knowlarity.extension?'<p> Dial Extension: '+t.response.knowlarity.extension+'</p><div class="phone-icon-panel"></div></div>':'')+'<div class="phone-appointment-details"><p>In case you are unable to get connected, Please call Bookdrappointment.com Helpline No : 022 49246246 (10 am to 7 pm, Monday to Saturday). Your missed call details will be provided to the Clinic.</p></div>';
                        $(e).html(i)
                    } else {
        ...
        $(e).toggle("fast", "linear", function() {
            0 == $(e).children("#select-date-slider-owl-demo").length && $.ajax({
                type: "POST",
                url: BASE_URL+"api/search/available_slots",
                dataType: "json",
                data: {
                    clinic_id: a,
                    doctor_id: t,
            ...
            data: {
                ...
                pass: t
						...
						beforeSend:function(e){
							$(".Bluelogin-doctor-bt").css("background",'#1a9f46 url("/static/images/ajax-loader.gif") no-repeat scroll 80% 45%');
							$("#login_error_msg").html('');
						},
            success: function(e) {
                e.error ? ($("#login_error_msg").html(e.error), $("#login_error_msg").show()) : form.submit()
                $(".Bluelogin-doctor-bt").css("background",'#1a9f46 url("/static/images/icon9.png") no-repeat scroll 80% 45%');
            }
        })}
