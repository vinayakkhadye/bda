...
function EventManager(options) { // assumed to be a calendar
	...
	function removeEventSources() { //hack to remove all event sources
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				...
				'</div>' +
				(isResizable ?
					'<div class="fc-resizer" style="display:none"/>' :
					''
					) +
			'</a>';
	},
...
$.extend(TimeGrid.prototype, {
	...
	slatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			axisHtml =
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" style="line-height: 30px;">' +
					//((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
						//'<span>' + // for matchCellWidths
							htmlEscape(calendar.formatDate(slotDate, view.opt('axisFormat'))) +
					//	'</span>' :
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
					...
					(event.title ?
						'<div class="fc-title">' +
							htmlEscape(event.title) + (event.patient_id ? '' : '&nbsp;&nbsp;<span class="glyphicon glyphicon-globe" aria-hidden="true"></span>')  +
						'</div>' :
						''
						) +
				'</div>' +
