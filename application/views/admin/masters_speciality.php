...
<html lang="en">
<head>
  <title>Speciality Master | BDA</title>
...
<form class="form-inline" method="POST" data-toggle="validator">
  ...
  <div class="form-group">
    ...
    <select name="new_record_status" class="form-control" required >
	    ...
	    <option value="2">Featuered</option>
...
<form class="form-inline" method="GET" action="">
  ...
  <div class="form-group">
    ...
    <select name="status" class="form-control">
	    <option value="">Select</option>
	    <option value="2" <?=(isset($_GET['status']) && $_GET['status']==2)?'selected':''?>>Featuered</option>
...
<form name="edit_records" id="edit_records" method="post" class="" >
    ...
    <div class="btn-group PB5">
      <button type="submit" name="submit" value="Feature"class="btn btn-primary">Feature</button>
    ...
    <td>
      <?php if($row->status == '1'): ?>Enabled<?php elseif($row->status == '2'): ?>Featured<?php elseif($row->status == '0'): ?>Disabled<?php else: ?>Deleted<?php endif; ?>
    </td>
    <td>
...
$(".sort-field").on('change', function()
	{
		...
		$.ajax(
		{
			url:'/bdabdabda/masters/update_sort/speciality',
			type:"POST",
			data:{
				'recordid':recordid,
				'sortvalue':sortvalue
...
$(".name-field").on('change', function()
	{
		...
		$.ajax(
		{
			url:'/bdabdabda/masters/update_name/speciality',
			type:"POST",
			data:{
				'recordid':recordid,
