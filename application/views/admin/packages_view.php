...
<form class="form-inline" name="seach_doctors" action="/bdabdabda/packages">
  ...
  <div class="form-group">
    <label class="control-label">Status : </label>
		...
		<option value="1" <?=(isset($status) && $status == 1)?'selected="selected"':''?>>In Use Package</option>
		<option value="0" <?=(isset($status) && $status == 0)?'selected="selected"':''?>>Sub Package</option>
		<option value="-1" <?=(isset($status) && $status==-1)?'selected="selected"':''?>>Expired Package</option>
		</select> 
  </div> 
  <div class="form-group">
...
<td>
  <?php if($row->status==1){ echo '<span class="label label-success">In Use</span>';}else if($row->status==0){ echo '<span class="label label-info">sub package</span>';}else if($row->status==-1){ echo '<span class="label label-danger">Expired Package</span>';}?>
</td>
<td>
...
<td>
<a href="/bdabdabda/packages/add_package/<?php echo $row->user_id; ?>" style="text-decoration: none;"><input type="button" value="Add Package" id="<?php echo $row->user_id; ?>" class="btn-xs btn-default"  /></a>
</td> 
<td>
