...
<button type="submit" name="duplicate" value="duplicate" class="btn btn-primary">Duplicate </button>
</div>
</div>
...
{?>
  ...
  <td>
    <a href="/bdabdabda/manage_doctors/viewprofile/<?php echo $row->doctor_id; ?>" <?=($row->is_ver_reg==2)?'class="red"':'' ?> ><?php echo ucfirst($row->doctor_name);?></a>
  </td>
  <td>
  <?php 
...
function refresh_appointment(id){
	$.ajax({
		...
		success: function(resp){
			...
			document.title = 'new ' + size+' |  Pending Appointments | BDA';
			var html = '';
			var  scheduled_time ='';
			for(i in resp)
			{
			...
			if(resp[i].confirmation==0){ 
			confirmation = 'Pending'; 
			}else if(resp[i].confirmation==1){ 
			confirmation = 'Confirmed'; 
			}else if(resp[i].confirmation==2){ 
			confirmation = 'In Process'; 
			}else if(resp[i].confirmation==3){ 
			confirmation = 'Duplicate'; 
			}
			
			registered	=	 '';
			if(resp[i].is_ver_reg==2){
				registered	=	 'red';
			}
			
			scheduled_time	=	resp[i].scheduled_time.replace(" ", "T");
			scheduled_time = new Date(scheduled_time).toUTCString();
			scheduled_time = new Date(scheduled_time);
			scheduled_time = scheduled_time.format("dd-mm-yyyy hh:MM tt");

			html = '<tr class="doc_details_row bgred" ><td class="check"><input type="checkbox" id="appointment_id_'+resp[i].id+'" name="appointment_id['+resp[i].id+']" class="rowcheck"></td><td class="'+registered+'">'+resp[i].doctor_name+'</td><td class="">'+dr_number+'</td><td class="">'+resp[i].city_name+'</td><td class="">'+resp[i].patient_name+'</td><td class="">'+resp[i].mobile_number+'</td><td class="">'+scheduled_time+'</td><td>'+status+'</td><td>'+confirmation+'</td> <td class=""><label>'+is_verified+'</label> </td><td><a href="/bdabdabda/appointments/view_appointment/'+resp[i].id+'"><span title="view appointment" class="glyphicon glyphicon-edit"></span></a><a "javascript:;" onclick="show('+resp[i].id+')" ><span title="view Sub Part" class="glyphicon glyphicon-menu-down"></span></a></td></tr>';
