...
<?php if(is_array($url_data) && sizeof($url_data)>0 ){foreach($url_data as $key=>$url){ ?><url><loc><?=$url['url'];?></loc><lastmod><?=$url['lastmod'];?></lastmod><changefreq>weekly</changefreq><priority><?=$url['priority'];?></priority></url><?php }}?>
<?php echo '</urlset>'; ?>