...
<form id="appointment_form" method="POST">
  <div class="dhx_cal_ltitle" style="cursor: pointer;">
    ...
    <input type="hidden" id="clinic_id" name="clinic_id" value="<?php echo $scheduler_clinic_id; ?>" />
  ...
  <div  style="height: 251px;">
    ...
    <div class="dhx_wrap_section">
      ...
      <div  style="float: left;">
        <input type="text" name="patient_contact_no" id="patient_contact_no" class="form-control required isEditable" style="width: 124px;" autocomplete="off" maxlength="10"/>
      </div>
...
<script type="text/javascript">
  ...
  {	
    ...
    var calendar =$('#calendar').fullCalendar({
      ...
      eventDrop: function(event, delta, revertFunc) {
        ...
        if (!confirm("Are you sure you want to rescedule this appointment")) {
          ...
          'clinic_number':'<?=$doctor_data['clinic_contact_number']?>',
          're_date':datebits[0],
          're_time':datebits[1],
   ...
   /* $('#clinics_dd').on('change',function(){
      ...
      //renderEvents(currentView);
		...
		$('#doctors_dd').on('change',function(){
			if($(this).val())
			{
      $.ajax({
              type: "POST",
              url: "<?php echo BASE_URL;?>api/scheduler/change_doctor_id",
              data: {'id':$(this).val(), 'duration':$(this).find('option:selected').attr("duration"),'clinic_id':$(this).find('option:selected').attr("clinic_id")},
              dataType: "json",
              beforeSend:function(){
                loading(); 
              },
  ...
  function renderEvents(view){
    if(view == "month"){
      ...
      if($('#calendar').fullCalendar( 'clientEvents') == "") {
        $('#calendar').fullCalendar( 'addEventSource', '<?php echo BASE_URL;?>api/scheduler/get_appointments_count?view='+view+'&did=<?php echo $doctor_id; ?>&cid='+$('#current_clinic_id').val());
      }
    }
    if(view == "agendaWeek"){
      $('#calendar').fullCalendar('removeEvents');
        ...
        $('#calendar').fullCalendar('refetchEvents');
      if($('#calendar').fullCalendar( 'clientEvents') == "") {
        $('#calendar').fullCalendar( 'addEventSource', '<?php echo BASE_URL;?>api/scheduler/get_appointments?view='+view+'&did=<?php echo $doctor_id; ?>&cid='+$('#current_clinic_id').val());
      }	
    }
    if(view == "agendaDay"){
      $('#calendar').fullCalendar('removeEvents');
    ...
    /*if( !$('#clinics_dd').length )	// use this if you are using id to check
    {
        ...
        var clinics = eval('<?php echo $clinics; ?>');
            $('.fc-left').append("<select id='clinics_dd' class='form-control' style='background-color: #f6f6f6; padding: 3px; width: 184px;'> <option duration='"+clinics[0].duration+"' value='0' doctor_id='0'>All Clinics </option> <select>");

        var options = $("#clinics_dd");
        $.each(clinics, function() {
            options.append($("<option />").val(this.id).text(this.name).attr('duration',this.duration).attr('doctor_id',this.doctor_id));
        });
    }*/
    if( !$('#doctors_dd').length )	// use this if you are using id to check
    {
        // Add the "dropdowns" to the day headers
        var doctors = eval('<?php echo $doctors; ?>');
            $('.fc-left').append("<select id='doctors_dd' class='form-control' style='background-color: #f6f6f6; padding: 3px; width: 184px;'> <option duration='"+doctors[0].duration+"' value='0' doctor_id='0'>Select Doctor </option> <select>");

        var options = $("#doctors_dd");
        $.each(doctors, function() {
            options.append($("<option />").val(this.doctor_id).text(this.doctor_name).attr('duration',this.duration).attr('clinic_id',this.clinic_id));
        });
    }		
    ...
    //$('#clinics_dd').val(<?php echo $scheduler_clinic_id; ?>);
		$('#doctors_dd').val(<?php echo $doctor_id; ?>);
  ...
  function delete_appointment(){
    ...
    $.ajax({
            ...
            data: {'appt_id':appt_id,'contact':contact,'doctor_id':<?php echo intval($doctor_id); ?>,'clinic_id':<?php echo intval($scheduler_clinic_id); ?>},
            beforeSend:function(){
              loading();
            },
            success: function(data) {
