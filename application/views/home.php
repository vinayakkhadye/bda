...
<body>
  ...
  <div id="banner">
      ...
      <?php if(isset($health_utsav['data']) && is_array($health_utsav['data']) && sizeof($health_utsav['data'])>0){ ?>
      <div class="cf thumb-slider">
	      <?php if(isset($health_utsav['category_id']) && $health_utsav['category_id']==1){ ?>
      	<a href="<?=$health_utsav['data'][0]['url']?>" title="<?=$health_utsav['data'][0]['title']?>">
	        <img src="<?=BASE_URL.$health_utsav['data'][0]['image']?>" alt="<?=$health_utsav['data'][0]['title']?>" style="width:100%">
        </a>
        <?php }else if(isset($health_utsav['category_id']) && $health_utsav['category_id']==2){ ?>
      	<a href="<?=$health_utsav['data'][0]['url']?>" title="<?=$health_utsav['data'][0]['title']?>">
	        <img src="<?=BASE_URL.$health_utsav['data'][0]['image']?>" alt="<?=$health_utsav['data'][0]['title']?>" style="width:49%">
        </a>
        <span style="width:20%"></span>
        <a href="<?=$health_utsav['data'][1]['url']?>" title="<?=$health_utsav['data'][1]['title']?>">
        <img src="<?=BASE_URL.$health_utsav['data'][1]['image']?>" alt="<?=$health_utsav['data'][1]['title']?>" style="width:49%">
        </a>
        <?php } ?>
      </div>
      <?php } ?>
