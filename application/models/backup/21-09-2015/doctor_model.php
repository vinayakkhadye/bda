...
class doctor_model extends CI_Model
{
	...
	function get_patients_by_id($doctorid)
	{
		...
		$this->db->where("dpm.doctor_id='".$doctorid."'", NULL, FALSE);
	...
	function getDoctorBySpecialityId($a = array())
	{
		...
		if(isset($a['location_in']) && !empty($a['location_in']))
		{
			...
			$this->db->order_by("doc.`city_sponsored` DESC, FIELD ( `location_id`,$locstr),doc.`sponsored` DESC, doc.`paid` DESC, doc.`sort` DESC, doc.`image` DESC, doc.`is_ver_reg` DESC");
