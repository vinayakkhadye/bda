...
class mail_model extends CI_Model
{
	...
	function appointment_rescheduled($to_email, $to_name,$a=array())
	{
		...
		$subject = 'Appointment Rescheduled with Dr. '.(isset($a['dr_name'])?$a['dr_name']:'').$fancy_date;
		return $this->sendmail($to_email, $to_name, $subject, $html);
		#------------
	}
	...
	function patient_appointment_feedback($to_email,$to_name,$a=array())
	{
		$html = $this->load->view('mailers/patient_appointment_feedback',array("name"=>$to_name,'doctor_id'=>$a['doctor_id'],'email'=>$to_email,'doctor_name'=>$a['doctor_name']), true);
		$subject = 'Appointment Feedback Bookdrappointment.com';
		return $this->sendmail($to_email, $to_name, $subject, $html);
		#------------
	}

	function appointment_delay($to_email,$to_name,$a=array())
	{
		$a['name']	=	$to_name;
		$a['email']	=	$to_email;
		$html = $this->load->view('mailers/appointment_delay',$a, true);
		$subject = 'Appointments Procees Delay Bookdrappointment.com';
		return $this->sendmail($to_email, $to_name, $subject, $html);
	}	
