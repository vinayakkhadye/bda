...
//		exit;
	...
	function update_doctor_professional_details($post, $filename_path, $userid)
	{
		$this->load->model('user_model');
		$userimg = $this->user_model->get_all_userdetails($userid);
//		print_r($post['speciality']);exit;
		...
		if($filename_path == NULL)
		{
			$data = array(
				...
				'other_specialization'  =>	$specialization_other,*/
				'qualification'      =>	$degree,
				'other_qualification'=>	$degree_other,
...
//		echo $this->db->last_query();
	...
	function getDoctorBySpecialityId($a = array())
	{
		...
		if(isset($a['location_in']) && !empty($a['location_in']))
		{
			...
			$this->db->order_by("FIELD ( `location_id`,$locstr),doc.`sponsored` DESC, doc.`paid` DESC, doc.`sort` DESC,doc.`is_ver_reg` DESC, doc.`image` DESC");
