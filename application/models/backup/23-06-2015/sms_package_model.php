...
{	
	...
	function active_communication_for_user($communication_trans_id,$communication_text,$health_checkup_days,$occasion_name,$occasion_date)
	{
	...
	function get_communication_recipients_for_user($communication_id)
	{
		if($communication_id)
		{
			$data = array(
			...
			$query	=	$this->db->get_where('communication_transaction_recipients', $data);
			if($query->num_rows() > 0)
			{
				return $query->row();
			}
	...
	function get_doctor_reminder($doctor_id)
	{
		if($doctor_id)
		{
			...
			$this->db->join('reminder_doctor_appt_map rdap', 'rdp.id=rdap.reminder_doc_appt_id','left');
