...
class doctor_model extends CI_Model
{
	...
	function insert_clinic_photos($post,$clinicid,$doctorid,$clinic_details)
	{
		$img1 = $post['clinicphotoimg1'];
		...
		{
			$photodata=array();
			$photodata[0]['name'] = $imgname1;
			$photodata[0]['image'] = $img1;
			$photodata[1]['name'] = $imgname2;
			$photodata[1]['image'] = $img2;
			$photodata[2]['name'] = $imgname3;
			$photodata[2]['image'] = $img3;
			$photodata[3]['name'] = $imgname4;
			$photodata[3]['image'] = $img4;
			$photodata[4]['name'] = $imgname5;
			$photodata[4]['image'] = $img5;
			$images = NULL;
			foreach($photodata as $key=>$photos)
			{
				# base64 image upload code
				$this->media_model->upload_type		=	'base64';
				$this->media_model->content_type	=	'clinic';
				$this->media_model->base64_data		=	$photos['image'];
				$this->media_model->file_name			=	$photos['name'];
				$file_data												=	$this->media_model->upload();
				if($file_data[0])
				{
					$images[$key]	=	$file_data[1];
				}
			}
			
			$clinic_images	=	array(0=>"",1=>"",2=>"",3=>"",4=>"");
			if(isset($clinic_details['images']) && !empty($clinic_details['images']))
			{
				$clinic_images	=	explode(",",$clinic_details['images']);
			}
			foreach($clinic_images as $cl_key=>$cl_val)
			{
				if(isset($images[$cl_key]))
				{
					$clinic_images[$cl_key]	=	$images[$cl_key];
				}
			}

			$imagedata = array('image'=>	implode(",",$clinic_images));
	...
	function update_clinic($userid, $doctorid, $clinicid, $post,$clinic_details=array())
	{
		$timings = json_encode(array(
				array(
					array(
		...
		$locality	=	0;
		$other_locality	=NULL;
		if(isset($post['locality']))
		{
			if(is_numeric($post['locality']))
			{
				$locality	=	$post['locality'];
			}
			else
			{
				$other_locality	=	$post['locality'];
			}
		}
		if(isset($post['other_locality']))
		{
			$other_locality	=	$post['other_locality'];
		}
		...
		)
		{
			...
			$this->insert_clinic_photos($post,$clinicid,$doctorid,$clinic_details);
		}
	}
	...
	function get_doctor_data($userid=0,$doctor_id=0,$a=array())
	{
		if($userid)
		{
		...
		if(isset($a['hospital_id']) && !empty($a['hospital_id']))
		{
			$search_array['hospital_id']	=	$a['hospital_id'];
		}
		
		if(isset($search_array) && sizeof($search_array)>0)
		{
			$query = $this->db->get_where('doctor', $search_array);
			if($query->num_rows() >= 1)
			{
				if(isset($a['hospital_id']) && !empty($a['hospital_id']))
				{
	...
	{
		$this->db->select('id, name, doctor_id, duration, address, contact_number, knowlarity_number, knowlarity_extension');
		$this->db->from('clinic');
	...
	function get_patients_by_id($doctorid=0,$hospital_id=0)
	{
		...
		if($doctorid)
		{
			$this->db->where("p.doctor_id='".$doctorid."'", NULL, FALSE);
		}
		if($hospital_id)
		{
			$this->db->where("p.hospital_id='".$hospital_id."'", NULL, FALSE);
		}
	...
	function getDoctorBySpecialityId($a = array())
	{
		...
		if(isset($a['location_in']) && !empty($a['location_in']))
		{
			...
			$this->db->order_by("doc.`city_sponsored` ASC, FIELD ( `location_id`,$locstr),doc.`sponsored` ASC, doc.`paid` ASC, doc.`sort` ASC, doc.`image` DESC, doc.`is_ver_reg` DESC");
	...
	function getDoctorByName($a = array())
	{
		...
		{
			$whereArray['cli.health_utsav'] = $a['health_utsav'];
		}
		if(isset($a['health_utsav_place']) && !empty($a['health_utsav_place']))
		{
			$whereArray['cli.health_utsav_place'] = $a['health_utsav_place'];
		}
	...
	function saveAppointment($a = array())
	{
		$data = array(
			...
			'patient_id'   		=> isset($a['patient_id'])?$a['patient_id']:NULL,
