...
class adminmasters_model extends CI_Model
{
	...
	function get_raw_sms_log_list($limit, $start, $search = array())
	{
		if(isset($search['status']) && strlen($search['status']) > 0)
		{
			$this->db->where('status', $search['status']);
		...
		if(isset($search['mobile_number']) && !empty($search['mobile_number']))
		{
			$this->db->like('mobile_number', trim($search['mobile_number']), 'both');
		}
		
		if(isset($search['message']) && !empty($search['message']))
		{
			$this->db->like('message', trim($search['message']), 'both');
		}
		
		if(isset($search['response']) && !empty($search['response']))
		{
			$this->db->like('response', trim($search['response']), 'both');
		}
		
		if(isset($search['created_on_start']) && !empty($search['created_on_start']) 
		&& isset($search['created_on_end']) && !empty($search['created_on_end']))
		{
			$this->db->where("created_on BETWEEN '".date("Y-m-d 00:00:00",strtotime($search['created_on_start']))."' and '".date("Y-m-d 23:59:59",strtotime($search['created_on_end']))."'");
		}
		
		
		$this->db->from('raw_sms_log');
		$this->db->limit($limit, $start);
