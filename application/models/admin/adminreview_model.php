...
class adminreview_model extends CI_Model
{
	...
	function insert_review($doctorid, $message, $rating, $name, $email, $image, $status=0,$added_on='')
	{
		$data = array(
		'doctor_id'	=>	$doctorid,
		'name'		=>	$name,
		...
		'rating'	=>	$rating,
		'status'	=>	$status,
		'added_on'=>	($added_on)?date("Y-m-d H:i:s",strtotime($added_on)):date("Y-m-d H:i:s")
		);
