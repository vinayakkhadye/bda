...
class adminappointments_model extends CI_Model {
    ...
    function get_appointments($a = array()) {
        ...
        if (!empty($a['doctor_id'])) {
            $whereArray['appointment.doctor_id'] = $a['doctor_id'];
        }
        if (!empty($a['name'])) {
        ...
        if(isset($a['order_by']) && !empty($a['order_by']))
        {
            $this->db->order_by($a['order_by']);
				...
				{
					$this->db->order_by('appointment.added_on desc');
				}
...
//		echo $status;exit;
    ...
    function get_all_scheduled_appointments($a=array()) {
				$new_appt_data	= false;
        $this->db->select('appointment.*, clinic.contact_number as clinic_contact_number,  city.name as city_name, doctor.name as doctor_name, doctor.contact_number as doc_contact_number, doctor.is_ver_reg');
        $this->db->from("appointment");
        $this->db->join('doctor', 'doctor.id = appointment.doctor_id');
        $this->db->join('city', 'appointment.city_id = city.id');
        $this->db->join('clinic', 'appointment.clinic_id = clinic.id');
				...
				{
					$this->db->order_by("appointment.scheduled_time asc");#appointment.id desc
				}
        $this->db->where($whereArray);
    ...
    function get_all_inprocess_appointments() {
        ...
        $whereArray = array('appointment.status' => 1, 'appointment.confirmation' => 2);
				$this->db->order_by("appointment.revisited_date asc,appointment.scheduled_time asc");
        $this->db->where($whereArray);
...
//    }
    ...
    function max_appointement_id(){
        ...
        function getNewPosts($max_id){
            ...
            $this->db->select('appointment.*, clinic.contact_number as clinic_contact_number, doctor.contact_number as doc_contact_number, city.name as city_name, doctor.name as doctor_name, doctor.is_ver_reg');
            $this->db->from("appointment");
            $this->db->join('doctor', 'doctor.id = appointment.doctor_id');
            $this->db->join('city', 'appointment.city_id = city.id');
             $this->db->join('clinic', 'appointment.clinic_id = clinic.id');
