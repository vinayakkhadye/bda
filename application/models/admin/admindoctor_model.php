...
//		$this->db->_protect_identifiers = FALSE;
	...
	function update_clinic($doctorid, $clinicid, $post)
	{
		...
		$data = array(
			...
			'updated_on'       =>	date('Y-m-d h:i:s'),
			'is_number_verified'=>	empty($post['is_number_verified']) ? 0 : $post['is_number_verified'],
			'knowlarity_number'=>	empty($post['knowlarity_number']) ? NULL : $post['knowlarity_number'],
			'knowlarity_extension'=>	empty($post['knowlarity_extension']) ? NULL : $post['knowlarity_extension'],
			'longitude'  		   	=>	isset($latlong[1]) ? $latlong[1] : NULL,
			'latitude' 	 	  		=>	isset($latlong[0]) ? $latlong[0] : NULL,
...
//		echo $this->db->last_query();
	...
	function get_last_knowlarity_extension()
	{
		$sql	=	"SELECT `knowlarity_number` as 'number',MAX(`knowlarity_extension`)+1 as 'extension' FROM clinic  WHERE `knowlarity_number` IS NOT NULL 
		GROUP BY `knowlarity_number`";
