...
class adminsetting_model extends CI_Model
{
	...
	function report($data)
	{
		...
		$this->db->select('`appointment`.`date`, DATE(appointment.added_on ) AS added_on , `city`.`name`,`doctor`.`speciality`,`appointment`.`status`,`appointment`.`from_app`, count(*) as total_appointment');
		$this->db->from('appointment');

		 // $whereArray="";
		if(!empty($data['city'])){
		  if ($data['city'] != 'all'){ $whereArray['appointment.city_id'] = $data['city']; }
		}
	...
	function doctor_list_pass($data)
	{
		$this->db->select('DISTINCT(appointment.doctor_id) as id ,doctor.name as name');
		$this->db->from('appointment');
		// $this->db->like('doctor.name',$data);
		$this->db->like('doctor.name',$data);
		$this->db->join('doctor','doctor.id = appointment.doctor_id');
		$query = $this->db->get();
		// echo $this->db->last_query();
   ...
   {
		 #print_r($data);exit;
   	$this->db->select('`appointment`.`added_on`,`appointment`.`patient_name`,`appointment`.`mobile_number`, `doctor`.`name` as dr_name, `city`.`name` as city_name,clinic.contact_number as clinic_contact_number,doctor.contact_number as doc_contact_number,`appointment`.`scheduled_time`,  `appointment`.`status`,`appointment`.`confirmation`');
   	$this->db->from('appointment');
		...
		if(!empty($data['city']))
		{
		  if ($data['city'] != 'all'){$this->db->where("appointment.city_id",$data['city']);}
		}
		...
		$this->db->order_by('appointment.added_on asc');
		$query= $this->db->get();
		if($query->num_rows() > 0)	
		{
			$rs= $query->result_array(); 
