...
class appointment_model extends CI_Model
{
	...
	function get_all_appointments($limit=NULL, $start=NULL,$type='',$patient_id=0,$doctor_id=0)
	{
		$this->db->select('appointment.id, appointment.patient_name,appointment.mobile_number as patient_contact,appointment.patient_id,appointment.scheduled_time, appointment.patient_email, appointment.patient_gender, appointment.doctor_id, appointment.clinic_id, appointment.city_id, appointment.reason_for_visit, appointment.date, appointment.time, appointment.mobile_number, appointment.consultation_type, appointment.status, appointment.confirmation, appointment.added_on, city.name as city_name, location.name as locality_name, doctor.user_id as user_id,doctor.name as doctor_name,doctor.image as doctor_image,doctor.gender as doctor_gender, doctor.speciality, doctor.other_speciality, clinic.other_location, clinic.contact_number as clinic_number,clinic.duration,clinic.name as clinic_name');#, speciality.name as speciality_name
		...
		if($type=='upcoming')
		{
			$this->db->where('appointment.scheduled_time > ', date("Y-m-d H:i:s"));
			$this->db->order_by('appointment.scheduled_time', 'asc');
		}
		else if($type=='past')
		{
			$this->db->where('appointment.scheduled_time < ', date("Y-m-d H:i:s"));
			$this->db->order_by('appointment.scheduled_time', 'desc');
		...
		if($patient_id)
		{
			$this->db->where('appointment.user_id ', $patient_id); #patient_id changed to user_id
		}
		if($doctor_id)
		{
	...
	function get_scheduler_appointment_byId($appointment_id){
		...
		$this->db->select('appointment.id, patient_name, patient_email, patient_gender,patient.image as patient_image, reason_for_visit, appointment.mobile_number, appointment.clinic_id,appointment.scheduled_time, DATE, TIME, clinic.duration, patient.id as patient_id, patient.user_id, patient.address, patient.dob');
		$this->db->from('appointment');
		$this->db->join('doctor', 'appointment.doctor_id = doctor.id');
		$this->db->join('clinic', 'appointment.clinic_id = clinic.id');
		$this->db->join('patient', 'appointment.patient_id = patient.id','left');
...
//		echo $this->db->last_query();exit;
	...
	function get_appointments_by_status($datetime,$doctor_id,$clinic_id,$status=1)
	{
		$this->db->select('appointment.id');
		$this->db->from('appointment');
		$this->db->where('scheduled_time', $datetime);
		$this->db->where('doctor_id', $doctor_id);
		$this->db->where('clinic_id', $clinic_id);
...
//		echo $this->db->last_query();
	...
	{
		$data = array("status" => 0,"confirmation" => 0);
	...
	function get_appointments_by_date_range($from, $to,$doctor_id='',$clinic_id='',$confirmation=0)
	{
		#$this->db->select('id');
		$this->db->from('appointment');
		if($doctor_id){
