...
class patient_model extends CI_Model
{
	...
	function update_patient_by_userid($user_id,$details,$image=''){
		if($user_id)
		{
			...
			if(isset($details['gender']) && !empty($details['gender']))
			{
				$patient['gender'] = $details['gender'];
			}

			if(isset($details['dob']) && !empty($details['dob']))
			{
			...
			if(isset($patient) && sizeof($patient)>0 && $user_id)
			{
	...
	function get_patientid_byuserid($userid,$doctorid='')
	{
		if($userid)
		{
			$srcarr['user_id']	=	$userid;
		}
		if($doctorid)
		{
			$srcarr['doctor_id']	=	$doctorid;
		}
		$srcarr['status']	=	1;
		
		$query = $this->db->get_where('patient', $srcarr);
		if($query->num_rows() >= 1)
		{
			return $query->row('id');
		}
	...
	function get_patientid_byemail($email,$doctorid='')
	{
		if($email)
		{
			$srcarr['email']	=	$email;
	...
	function get_patient_list($a=array())
	{
		...
		{
			$this->db->where("p.doctor_id='".$a['doctor_id']."'", NULL, FALSE);
		}
		...
		{
			$srcArr['p.hospital_id'] = $a['hospital_id'];
		}
/*		if(isset($a['clinic_id']) && !empty($a['clinic_id']))
		{
			$srcArr['p.clinic_id'] = $a['clinic_id'];
		}
*/
		$srcArr['p.status'] = 1;
		$this->db->distinct();
		$this->db->select('`p`.`id`, `p`.`user_id`, `p`.`name`, `p`.`mobile_number`, `p`.`address`, `p`.`dob`, `p`.`email`, `p`.`gender`, `p`.`image`,`p`.`doctor_id`,`p`.`created_on`');#,`apt`.`clinic_id`, `apt`.`doctor_id` AS `apt_doctor_id`
