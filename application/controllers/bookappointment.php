...
class Bookappointment extends CI_Controller
{
	...
	{
		$this->load->model(array('doctor_model','sendsms_model','appointment_model'));
		...
		if($insert_id)
		{
			...
			$rs            = $this->db->query("INSERT INTO `caller_verify`(`number`,`appointment_id`)VALUES ('".$caller_number."','".$insert_id."')");
	...
	function mailp()
	{
		$this->load->view('mailers/practo_mail',array());	
		
	}
	function mailb()
	{
		$this->load->view('mailers/Appointment_Requested',array());	
	}
}
