...
class Profile extends CI_Controller
{
	...
	public function short_url($tiny_url)
	{
		$sql	=	"SELECT doctor.`tiny_url`,doctor.`url_name`,city.`name` FROM doctor JOIN clinic ON doctor.id= clinic.`doctor_id` 
		JOIN city ON clinic.`city_id`=city.id WHERE tiny_url='".$tiny_url."'";
		$query	=	$this->db->query($sql);
		if($query->num_rows() >= 1)
		{
			$doctor = current($query->result_array());
			redirect(url_string("/".$doctor['name']."/profile/".$doctor['url_name'], 'location', 301));exit;
		}
		else
		{
