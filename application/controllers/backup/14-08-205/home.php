...
class Home extends CI_Controller
{
    ...
    public function index($params)
    {
        ...
        if (is_array($params) && sizeof($params) == 1) {
            $this->data['location']       = $this->common_model->getLocation(array(
                'status' => ACTIVE,
            ...
            $this->data['speciality']     = $this->common_model->getSpecialityByCity(array(
                'city_id' => $this->data['cityId'],
                'orderby' => 'name asc',
                'column' => array(
                    'name',
            ...
            $this->data['top_speciality'] = $this->common_model->getSpecialityByCity(array(
                'status' => 2,
                'column' => array(
                    'sp.id,sp.name,sp.display_name,sp.display_image,sp.url_name'
                ),
                'city_id' => $this->data['cityId'],
                'orderby' => 'sp.sort asc'
            ));
            ...
            $city_top_speciality = $this->common_model->getSpecialityByCity(array(
                'status' => 2,
                'column' => array(
                    'scp.city_id,sp.id,sp.name,sp.display_name,sp.display_image,sp.url_name'
                ),
                'city_id' => $city_id_array,
                'orderby' => 'scp.city_id,sp.display_name asc'
            ));
    ...
    public function search($params)
    {
        ...
        if (isset($speciality)) {
            ...
            $specialityArray              = $this->common_model->getSpeciality(array(
                'url_name' => $speciality,
                'limit' => 1
            ));
            ...
            if (isset($location) && !empty($location) && $location !== 'all') {
                $locArray = $this->common_model->getLocation(array(
                    'status' => ACTIVE,
                    'url_name' => $location,
                    'city_id' => $this->data['cityId'],
                    'limit' => 1,
        ...
        if (is_array($doctors) && sizeof($doctors) > 0) {
            $qualification_data = $this->common_model->getQualification(array(
                'column' => array(
                    'name',
                    'id'
                ),
            ...
            $speciality_data    = $this->common_model->getSpeciality(array(
                'column' => array(
                    'name',
                    'id'
                ),
