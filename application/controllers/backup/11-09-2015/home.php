...
class Home extends CI_Controller
{
	...
	public function index($params)
	{
		...
		if (isset($params) && is_array($params) && sizeof($params) == 1)
		{
			...
			$this->data['health_utsav'] = current($this->boxmanager_model->get_data('health_usav',$this->data['cityId']));
	...
	public function search($params)
	{
			...
			if (isset($speciality))
			{
					...
					if (isset($location) && !empty($location) && $location !== 'all')
					{
							...
							{
									echo $this->load->view('errors/page_missing', array(),true);exit;
									$searchArray['orderby'] = 'doc.city_sponsored DESC, doc.`sponsored` DESC, doc.`paid` DESC, doc.`sort` DESC, doc.`image` DESC, doc.`is_ver_reg` DESC';
							}
