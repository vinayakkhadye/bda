...
class Sitemap extends CI_Controller
{
	...
	/*function speciality_keywords()#$city_name,$city_id
	{
		...
		$sql	=	"SELECT s.name as 'speciality',l.name AS 'location_name',l.url_name AS 'location_url_name',c.name as 'city_name' FROM speciality_city_map scp INNER JOIN speciality s ON s.id=scp.speciality_id INNER JOIN city c ON c.id=scp.city_id INNER JOIN location l ON l.city_id=c.id WHERE l.url_name is not null and l.status=1 and s.status in(1,2) and scp.status=1 ORDER BY s.name,l.name limit 200000";
		$rs	=	$this->db->query($sql);#l.city_id = $city_id and 
		...
		if($rs->num_rows>0)
		{
			...
			foreach($result as $key=>$val)
			{
				if($val['location_url_name'])
				{
					fputcsv($output, array($val['speciality'].' in '.$val['location_name']." ".$val['city_name'],$val['speciality'].' in '.$val['location_name']));
				}
			}
		}
	}*/
	...
	function sitemap_profile_xml($city_name)
	{
		if($city_name)
		{
			$url_data	=	 array();
			$this->load->model(array('common_model'));
			$city 			= $this->common_model->getCity(array('limit'=>1,'column'=>array('id','name'),'name'=>reverse_url_string($city_name),'status'=>array(1,2)));
			$url_data	=	 array();
			if(is_array($city) && sizeof($city)>0)
			{
				$city_id	=	 $city[0]['id'];
				$sql	=	"SELECT DISTINCT doc.url_name FROM doctor doc JOIN clinic cli ON  doc.id=cli.doctor_id JOIN city c ON c.id=cli.city_id WHERE doc.status=1 AND cli.status=1 AND cli.city_id=$city_id";
				...
				if($rs->num_rows>0)
				{
						...
						if(is_array($result) && sizeof($result)>0)
						{
							foreach($result as $key=>$val)
							{
								...
								{
									$url_data[]	=	array(
																'url'=>BASE_URL.url_string($city_name).'/profile/'.$val['url_name'],
																'priority'=>'0.8',
																'changefreq'=>'weekly',
																'lastmod'=>date("Y-m-d")
															);
	...
	{
		$url_data[0]	=	array('url'=>BASE_URL,'priority'=>'1.0','changefreq'=>'daily','lastmod'=>date("Y-m-d"));
		$url_data[1]	=	array('url'=>BASE_URL."patient",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));
		$url_data[2]	=	array('url'=>BASE_URL."doctor-practice-management",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));		
		$url_data[3]	=	array('url'=>BASE_URL."about-us",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));
		$url_data[4]	=	array('url'=>BASE_URL."contact-us",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));
		$url_data[5]	=	array('url'=>BASE_URL."patient-faq",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));
		$url_data[6]	=	array('url'=>BASE_URL."doctor-faq",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));
		$url_data[7]	=	array('url'=>BASE_URL."privacy-policy",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));
		$url_data[8]	=	array('url'=>BASE_URL."terms-conditions",'priority'=>'0.8','changefreq'=>'weekly','lastmod'=>date("Y-m-d"));
		$this->load->view('website_sitemap_xml',array("url_data"=>$url_data));
	}
		...
		if($rs->num_rows>0)
		{
			...
			foreach($result as $key=>$val)
			{
				if($val['city_name'])
				{
					...
					$url_data[]	=	array('url'=>BASE_URL."sitemap-profile/".url_string($val['city_name']).".xml",'priority'=>'0.8','changefreq'=>'monthly','lastmod'=>date("Y-m-d"));
				}
			}
		}
		$this->load->view('website_sitemap_xml',array("url_data"=>$url_data));
