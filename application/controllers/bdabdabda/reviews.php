...
class Reviews extends CI_Controller
{
	...
	function search()
	{
		...
		$scharr= array('limit'  =>$config['per_page'],'offset' =>$page,'orderby'=>'reviews.updated_on desc');
	...
	function add_reviews()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			if(!empty($_POST['doctorid']) && !empty($_POST['username']) && !empty($_POST['message']) && !empty($_POST['rating']) && is_numeric($_POST['doctorid']))
			{
				...
				$this->adminreview_model->insert_review($_POST['doctorid'], $_POST['message'], $_POST['rating'], $_POST['username'], NULL, $filename_path, '1',$_POST['added_on']);
