...
class knowlarity extends CI_Controller
{
	...
	function call_phone_record()
	{	
		$this->data['current_tab'] = "knowlarity";
		$config['base_url'] = BASE_URL.'bdabdabda/knowlarity/call_phone_record?';
		$config['per_page'] = 30;
		if(count($_GET) > 1)
		{
			$config['base_url'] = $config['base_url'] . http_build_query($_GET, '', '&');
		...
		$page = !empty($_GET['start']) ? $_GET['start'] : 0;
		$config['total_rows'] = $this->adminknowlarity_model->call_phone_record_count();
		$this->pagination->initialize($config); 
		
		$this->data['flag']	= $this->adminknowlarity_model->call_phone_record($config['per_page'],$page); 
		$this->data['perms']	=	$this->perms;
		$this->load->view('admin/knowlarity_call_phone_record',$this->data);
	}
	
	function max_phone_call_record()
	{	 
		$max_call_record = $this->adminknowlarity_model->max_id_phone_call_record();
	...
	function new_phone_call_record()
	{
		$max_row	=	 $_POST['max_row'];  
		if($max_row)
		{
		$data	= $this->adminknowlarity_model->new_phone_row_call_record($max_row);
		}  
