...
class Manage_Doctors extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model(array('admin/admindoctor_model','doctor_model','location_model','page_model','user_model','doctor_details_model','common_model'));
	...
	function index()
	{
		...
		if(isset($this->data['results']))
		{
			foreach($this->data['results'] as $key=>$val)
			{
				...
				{
					if(isset($this->data['specialities'][$q_v]['name']))
					{
						$val->qual	.=	$this->data['specialities'][$q_v]['name'].", ";
					}
				}
	...
	function search()
	{
		...
		$scharr = array('limit'  =>$config["per_page"],'offset' =>$page,'orderby'=>'doc.updated_on desc');
	...
	function editbasicprofile($doctor_id = NULL)
	{
		...
		else
		{
			if(isset($_POST['submit']) && !empty($_POST['submit']))
			{
				...
				redirect('/bdabdabda/manage_doctors/editdetailedprofile/'.$doctor_id);
			}
	...
	function editdetailedprofile($doctor_id = NULL)
	{
		...
		else
		{
			...
			$data['services'] = $this->doctor_details_model->getServices(array('limit'=>500));

			$data['specializations'] = $this->common_model->getSpeciality(array('limit'=>1000,'column'=>array('id','name')));
			$data['qualification'] = $this->doctor_details_model->getQualification(array('limit'=>500));
			$data['college'] = $this->doctor_details_model->getCollege(array('limit'=>500));
			...
			$data['membership'] = $this->doctor_details_model->getMembership(array('limit'=>500));
			$data['council'] = $this->doctor_details_model->getCouncil(array('limit'=>500));
		...
		if(!empty($doctor_id) && is_numeric($doctor_id))
		{
			...
			if(isset($_POST['submit']) && !empty($_POST['submit']))
			{
				...
				if($this->form_validation->run() !== FALSE)
				{
					...
					if($id)
					{
						$this->admindoctor_model->update_doctor_userid($id, $doctor_id);
...
//						print_r($checklistenrty);
						...
						{
							echo '
							<div style="max-width: 600px;color: #555;margin: 5px 0 2px;text-align: left;">
								<input type="checkbox" class="resp-checkbox" '.$checked.' name="statuscheckbox['.$row->id.']" />'.$row->desc.'<br/>
							</div>
							';
							if($row->have_comment == 1)
							echo '
							<div style="max-width: 600px;text-align: left;">
								<input type="text" value="'.@$checklistenrty->comment.'" style="width: 95%; padding: 5px;" class="resp-text" name="checkboxcomment['.$row->id.']" placeholder="Comment" /><br/>
							</div><br/>
							';
						}
						elseif($row->type == '9')
						{
							echo '
							<div style="max-width: 600px;color: #555;margin: 5px 0 2px;text-align:center;">
								<textarea required="required" name="checkboxcomment['.$row->id.']" class="resp-textarea" id="'.$row->id.'" placeholder="Notes"  cols="40" rows="6">'.@$checklistenrty->comment.'</textarea>
							</div>
							';
						}
						echo '
							<input type="hidden" name="checklisttype['.$row->id.']" value="'.$row->type.'" />
						...
						if($row->category == 'doctor_approve')
						{
							echo '
							<center>
								<input style="height: 30px;" type="submit" class="save-form-btn" value="Save Changes" />
								<input style="height: 30px;" type="submit" class="submit-form-btn" value="Update Status" />
							</center>
							';
						}
						else
						{
							echo '
							<center><input style="height: 30px;" type="submit" class="submit-form-btn" value="Update Status" /></center>
							';
						}
				}
				else
...
//				echo $key . ' => ' . $value . ';  ';
				...
				if($value != '0')
				{
					...
					if($check == true)
					{
						if($category == 'doctor_approve')
						$status = '1';
						elseif($category == 'doctor_disapprove')
						$status = '-1';
						elseif($category == 'doctor_pending')
						$status = '0';
						elseif($category == 'doctor_delete')
						$status = '-2';
						else
						$status = NULL;
						if($status !== NULL)
