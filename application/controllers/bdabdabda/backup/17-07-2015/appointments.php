...
class Appointments extends CI_Controller
{
	...
	function __construct()
	{
		...
		#$this->perms	=	$this->perms[ADMIN_APPOINTMENTS];
	...
	function search()
	{
		...
		$scharr= array('limit'  =>$config["per_page"],'offset' =>$page,'orderby'=>'appointment.added_on desc');
	...
	public function appointment_confirmation_mail_msg($ids)
	{
		...
		foreach($ids as $id)
		{
			...
			"d.gender AS doctor_gender","apt.date AS appointment_date","apt.doctor_id","apt.time AS appointment_time","apt.patient_name AS patient_name","apt.patient_email AS patient_email","d.speciality","d.qualification","c.name AS 'clinic_name'","c.address AS 'clinic_address'","c.contact_number AS 'clinic_number'","c.knowlarity_number","c.knowlarity_extension","apt.reason_for_visit","apt.id AS 'appointment_id'","apt.mobile_number","l.name as 'location_name'")));
			...
			if(is_array($apt_data) && sizeof($apt_data) > 0)
			{
				...
				'clinic_contact'=>$apt_data['clinic_number']
				);

				#print_r($msgArray);
				$this->sendsms_model->send_appointment_confirmation_sms($apt_data['mobile_number'],$msgArray);
				
				$sql	=	"SELECT d.contact_number FROM doctor d JOIN appointment apt ON apt.`doctor_id`=d.id WHERE apt.doctor_id = ".$apt_data['doctor_id'];
				...
				if($user_rs->num_rows()>0)
				{
					...
					{
						$doc_msg_arr	=	array('name'=>$apt_data['patient_name'],
						'mobile_number'	=>	$apt_data['mobile_number'],
						'time'           =>date('dS M Y \a\t h:i a',strtotime($apt_data['appointment_date']." ".$apt_data['appointment_time'])),
						'clinic_name'    =>$apt_data['clinic_name'],
						'reason_for_visit'=>$apt_data['reason_for_visit'],
						);
						$this->sendsms_model->send_appointment_confirmation_doctor_sms($user_details->contact_number,$doc_msg_arr);
					}
				}
