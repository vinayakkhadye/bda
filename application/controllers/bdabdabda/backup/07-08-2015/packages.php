		...
		if($userid != NULL && is_numeric($userid))
		{
			if(isset($_POST['submit']))
			{
				if(!empty($_POST['start_date']) && !empty($_POST['end_date']))
				{
					...
					$chk = $this->adminpackages_model->check_doctor_package_eligibility($userid, $_POST['packageid']);

					if($chk === TRUE)
					{
						$insert_id	=	$this->adminpackages_model->insert_package($userid, $_POST['packageid'], $_POST['start_date'], $_POST['end_date']);
						if($insert_id)
						{
