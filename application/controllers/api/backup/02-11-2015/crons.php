...
WHERE appointment.`status` = 1 AND appointment.`confirmation` = 1 AND appointment.`scheduled_time` BETWEEN '".$start."' AND '".$end."'";
	...
	{
		$sql	=	"select id,caller_number,agent_number,call_id FROM call_phone_record WHERE response=0 AND counter<3 and call_status='Connected'";
		$query	=	$this->db->query($sql);
		$list = $query->result_array();
		...
		if($list)
		{
			foreach($list as $key=>$val)
			{
				...
				{
						$sql	=	"update call_phone_record set response=1 where id='".$val['id']."'";
						$query	=	$this->db->query($sql);
				}
				else
				{	
					$sql	=	"select doctor_id from clinic where contact_number	like	'%".$val['agent_number']."%' and status=1 limit 1";
					$query	=	$this->db->query($sql);
					...
					if($clinic && is_array($clinic))
					{
						...
						if($doctor && is_array($doctor))
						{
							...
							#echo $knowlarity_obd_api."<hr/>";
							file_get_contents($knowlarity_obd_api); ## remove this comment after knowlarity work is done
							#$sql	=	"update call_phone_record set response=1 where id='".$val['id']."'";
							#$query	=	$this->db->query($sql);
