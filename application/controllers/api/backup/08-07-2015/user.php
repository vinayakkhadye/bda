...
class User extends REST_Controller
{
	...
	function login_post()
	{
		$rs = array("message"=>"Please Provided type.","status" =>0);
	...
	public function imageupload_post()
	{
		...
		if(file_put_contents($filename_path, $decoded_pic))
		{
			...
			if($image_type)
			{
				$rs = array("image_data"=>$filename_path,"message"   =>"Image uploaded successfully new","status"    =>1);
			}
			else
			{
		...
		else if($user_id && $type == 1)
		{
			...
			if(is_array($user_id_arr) && sizeof($user_id_arr) > 0)
			{
				foreach($user_id_arr as $user_val)
				{
					...
					{
						$result[$user_val]['user'] = array("message"=>"user deleted successfully ","status" =>1);
					}
					else
					{
						$result[$user_val]['user'] = array("message"=>"no user found","status" =>1);
					}

					$sql = "update patient set user_id =  NULL where user_id = ".$user_val;
					$rs  = $this->db->query($sql);
					if($this->db->affected_rows() > 0){
						$result[$user_val]['patient'] = array("message"=>"patient deleted successfully ","status" =>1);
					}
					else
					{
						$result[$user_val]['patient'] = array("message"=>"no patient found ","status" =>1);
					}
					$this->db->trans_complete();
					if ($this->db->trans_status() === FALSE)
					{	
