...
WHERE appointment.`status` = 1 AND appointment.`confirmation` = 1 AND appointment.`scheduled_time` BETWEEN '".$start."' AND '".$end."'";
	...
	{
		$sql	=	"select caller_number,agent_number,call_id FROM call_phone_record WHERE response=0 AND counter<2";
		$query	=	$this->db->query($sql);
		$list = $query->result_array();
		...
		if($list)
		{
			foreach($list as $key=>$val)
			{
				$sql	=	"select doctor_id from clinic where contact_number	=	'".$val['agent_number']."' and status=1 limit 1";
				$query	=	$this->db->query($sql);
