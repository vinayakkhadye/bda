...
class Patient extends REST_Controller
{
	...
	function profile_post()
	{
		...
		$file_log = DOCUMENT_ROOT."logs/profile_log_".date("Y-m-d").".log";
		...
		if($user_id)
		{
			...
			$data	=	$this->user_model->get_all_userdetails($user_id);		
			...
			$data->email_notify	=	0;
			$data->sms_notify	=	0;
			$data->food_habits	=	NULL;
			$data->alcohol	=	NULL;
			$data->smoking	=	NULL;
			
			$data->ciggi_per_day	=	NULL;
			$data->tobacco_consumption	=	NULL;
			$data->status	=	1;
		}
		else if($patient_id)
		{
	...
	function updatebasicprofile_post()
	{
		...
		$file_log = DOCUMENT_ROOT."logs/updatebasicprofile_log_".date("Y-m-d").".log";
		$this->log_file = fopen($file_log, "a+"); 
		$this->log_message($this->log_file,json_encode($this->post()).NEW_LINE);
		...
		$check	=	$this->patient_model->update_patient($data,0,$user_id);
		if($check)
		{
			$rs = array("profile_data"=>"basic profile updated","message"=>"basic profile updated","status"=>1);
		}
		else
		{
				$rs = array("profile_data"=>"basic profile updated","message"=>"basic profile not updated","status"=>0);
		}
	...
	function save_appointment_post()
	{
		...
		if(empty($appointment_date) || empty($appointment_time) || empty($clinic_id) ||  empty($doctor_id) ||  empty($city_id) ||  empty($patient_gender))
		{
			$rs = array("message"=>"please provide appt_date,appt_time,clinic_id,doctor_id,city_id,patient_gender","status" =>0);
			$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code 
		}
		...
		if(!in_array($patient_gender,array('m','f','o','M','F','O')))
		{
			$rs = array("message"=>"Please Provide patient_gender as M,F,O OR m,f,o ","status" =>0);
			$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code 
		}
		...
		{
			$user	=	$this->user_model->get_all_userdetails_byemail($patient_email);
		}
	...
	function save_appointment_admin_post()
	{
		$appointment_date = date("Y-m-d",strtotime($this->post('appt_date')));
		$appointment_time = date("H:i:s",strtotime($this->post('appt_time'))); ;
		$clinic_id = $this->post('clinic_id');
		$doctor_id = $this->post('doctor_id');
