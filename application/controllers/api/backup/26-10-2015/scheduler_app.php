...
class Scheduler_app extends REST_Controller
{
	...
	function GetAppointmentById_post()
	{
		...
		if($appointment_id){
			...
			if($data && sizeof($data)>0){
				...
				$tmpData['clinic_id'] = ($data->clinic_id)?intval($data->clinic_id):NULL;
				$tmpData['patient_id'] = ($data->patient_id)?intval($data->patient_id):245;
				...
				$tmpData['patient_dob'] = date("d-m-Y",strtotime("-3 year"));
				if($data->dob)
				{
				...
				$tmpData['patient_image'] = ($data->patient_image)?IMAGE_URL.$data->patient_image:"https://www.bookdrappointment.com/static/images/./media/photos/Mar2015/p/854ab1aa10c6522c114792020ac552d4.JPEG";
  ...
  function DeleteBlockSlot_post()
	{
		$blocked_id = intval($this->post('block_slot_id'));
		if(empty($blocked_id))
		{
