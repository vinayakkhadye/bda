...
WHERE appointment.`status` = 1 AND appointment.`confirmation` = 1 AND appointment.`scheduled_time` BETWEEN '".$start."' AND '".$end."'";
	...
	{
		$sql	=	"select id,caller_number,agent_number,call_id FROM call_phone_record WHERE response=0 AND counter<2";
		$query	=	$this->db->query($sql);
		$list = $query->result_array();
			...
			foreach($list as $key=>$val)
			{
				...
				{	
					...
					if($clinic && is_array($clinic))
					{
						...
						{
							$knowlarity_obd_api	=	"http://etsdom.kapps.in/webapi/bda/api/bda_obd.py?customer_number=".$val['caller_number']."&doctor_name=".$doctor['name']."&doctor_id=".$clinic['doctor_id']."&auth_key=6842c1ee-ac05-4e90-8b6f-0d40e40240d4&call_id=".$val['call_id'];
	
							#echo $knowlarity_obd_api."<hr/>";
							#var_dump(file_get_contents($knowlarity_obd_api)); ## remove this comment after knowlarity work is done
							#$sql	=	"update call_phone_record set response=1 where id='".$val['id']."'";
							#$query	=	$this->db->query($sql);
