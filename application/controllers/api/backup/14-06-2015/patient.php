...
class Patient extends REST_Controller
{
	...
	function get_appointment_post()
	{
		...
		$data = $this->appointment_model->get_all_appointments($limit,$offset,$type,$patient_id);
		...
		if(is_array($data) && sizeof($data)>0)
		{
			foreach($data as $key=>$val)
			{
				...
				$rs[$bookedStartTime]['date'] = date("m/d/Y",$cur_time);
				...
				$rs[$bookedStartTime]['other_speciality'] = $val->other_speciality;
