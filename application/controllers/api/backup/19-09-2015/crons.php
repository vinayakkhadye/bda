...
WHERE appointment.`status` = 1 AND appointment.`confirmation` = 1 AND appointment.`scheduled_time` BETWEEN '".$start."' AND '".$end."'";
	...
	{
		/*$sql='SELECT added_on,patient_name,doctor.name doctor,
		CASE WHEN confirmation_date IS NULL THEN TIMESTAMPDIFF(MINUTE,added_on,NOW()) ELSE TIMESTAMPDIFF(MINUTE,added_on,confirmation_date) END AS time_minutes,appointment.status,appointment.confirmation 
		FROM appointment 
		JOIN doctor ON appointment.doctor_id = doctor.id 
		WHERE appointment.status < 2
		AND confirmation < 2 
		AND TIME(added_on) BETWEEN "09:00:00" AND "20:00:00" 
		AND DAYOFWEEK(added_on) > 1 
		AND added_on > (NOW() - INTERVAL 90 MINUTE) 
		AND (
		(confirmation_date IS NOT NULL AND TIMESTAMPDIFF(MINUTE,added_on,confirmation_date) > 30) 
    OR 
    (confirmation_date IS NULL AND TIMESTAMPDIFF(MINUTE, added_on, NOW()) > 30))';*/
		$sql='SELECT 	added_on,
					patient_name,
					doctor.name doctor,
					CASE WHEN confirmation_date IS NULL THEN TIMESTAMPDIFF(MINUTE,added_on,NOW()) ELSE TIMESTAMPDIFF(MINUTE,added_on,confirmation_date) END AS time_minutes,
					appointment.status,
