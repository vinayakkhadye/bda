...
class Search extends REST_Controller
{
	...
	public function speciality_post()
	{
		...
		$sql	=	"SELECT sp.id,sp.name,sp.display_name,sp.status,scp.speciality_id FROM `speciality` sp left join speciality_city_map scp on sp.id=scp.speciality_id and city_id=".$city_id." order by sp.status desc,sp.sort asc,sp.name asc";
		...
		if($query->num_rows>0)
		{
			...
			foreach($speciality as $key=>$val)
			{
				...
				$speciality_data[$key]['name']	=	$val['name'];
	...
	public function doctorlist_by_speciality_location_post()
	{
		...
		$this->load->model(array('common_model','doctor_model','location_model','reviews_model'));
		...
		if(is_numeric($location_id) && !empty($location_id))
		{
			...
			{
				$searchArray['orderby'] = 'doc.created_on desc';
			}
		}
		else
		{
		...
		if(is_array($doctors) && sizeof($doctors)>0)
		{
			...
			foreach($doctors as $dKey=>$dVal)
			{
				...
				}else if(isset($dVal['clinic_other_location']) &&!empty($dVal['clinic_other_location']) && isset($location_data[$dVal['clinic_other_location']]))
				{
					$doctors[$dKey]['clinic_location_name']	=	$location_data[$dVal['clinic_other_location']];
				}
				else
				{
				...
				if(!empty($doctors[$dKey]['clinic_latitude']) && !empty($doctors[$dKey]['clinic_longitude']) && !empty($device_latitude) && !empty($device_longitude)  )
				{
					$doctors[$dKey]['distance'] = number_format($this->location_model->distance_by_lat_lng($doctors[$dKey]['clinic_latitude'],$doctors[$dKey]['clinic_longitude'],$device_latitude,$device_longitude,"K"),2);
				}
				...
				else if($doctors[$dKey]['is_ver_reg']==1 || $doctors[$dKey]['is_number_verified']==1)
				{
					$doctors[$dKey]['appointment_type']	=	 'appointmentviaphone';
				}
				else				
				if($doctors[$dKey]['is_ver_reg']==0)
				{
		...
		if(is_array($doctors) && sizeof($doctors)>0)
		{
			...
			foreach($doctors as $dKey=>$dVal)
			{
				...
				if(isset($dVal['clinic_location_id']) &&!empty($dVal['clinic_location_id']))
				{
					if(isset($location_data[$dVal['clinic_location_id']]))
					{
						$doctors[$dKey]['clinic_location_name']	=	$location_data[$dVal['clinic_location_id']]['name'];
					}
					else
					{
		...
		if($query->num_rows > 0)
		{
			...
			foreach($date_range as $date_key=>$date_value)
			{
				...
				if(isset($time_data[$week_day][0]))
				{
					...
					{
						$time_array[$date_key]['time'][0]	=	array_filter($time_array[$date_key]['time'][0],array('Search','remove_past_time'));
						#print_r($time_array[$date_key]['time'][0]);exit;
					}
				}
				if(isset($time_data[$week_day][1]))
				{
					if(isset($appointment_data[$date_value]))
					{
					...
					if($current_date	==	$time_array[$date_key]['org_date'])
					{
						$time_array[$date_key]['time'][1]	=	array_filter($time_array[$date_key]['time'][1],array('Search','remove_past_time'));
						#print_r($time_array[$date_key]['time'][1]);exit;
					}
