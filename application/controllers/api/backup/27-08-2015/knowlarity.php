...
class Knowlarity extends REST_Controller
{
	...
	{ 	
		...
		$knowlarity_num = $this->get('knowlarity_number');
		...
		if($rs->num_rows() > 0)
			{
				$res = $rs->row_array() ;
				$rs = array("contact_number"=>$res['contact_number'],"message"  =>"successfull","status"	=>1);
				$this->response($rs, 200);// 200 being the HTTP response code		 
		 }	
		 else 
		 {
	...
	{
		$file_log = DOCUMENT_ROOT."logs/knowlarity_logs/pushcdr_".date("Y-m-d").".log";
		$this->log_file = fopen($file_log, "a+"); 	
		...
		if($call_id && $caller_number && $agent_number)
		{
			if($this->check_agent_number($agent_number))
			{
				...
				$rs  = $this->db->query($sql);
				$this->log_message($this->log_file," sql ".$sql.NEW_LINE);		
				if($rs)
				{
					$rs = array("message"=>"successfull","status" =>1);
				}
	...
	public function pushcdr_phone_get()
	{
		...
		$call_duration = trim($this->get('call_duration'));
		$extension     = trim($this->get('extension'));
		$knowlarity_number  = "+".trim($this->get('knowlarity_number'));
		$recording_url = trim($this->get('recording_url'));
		$call_id       = trim($this->get('call_id'));
		
		// $busy = $this->makeagent_available($agent_number,0);
		
		if($call_id && $caller_number && $knowlarity_number)
		{
			 
				$sql = "INSERT INTO `call_phone_record`(`date`,`time`,`caller_number`,`call_status`,`call_duraton`,`extension`,`knowlarity_number`,`recording_url`,`call_id`)
				VALUES ('".$date."','".$time."','".$caller_number."','".$call_status."','".$call_duration."','".$extension."','".$knowlarity_number."','".$recording_url."','".$call_id."')";
				$rs  = $this->db->query($sql);
				if($rs)
				{
	...
	public function verify_caller_get()
	{
		...
		$sql	=	"SELECT MINUTE(TIMEDIFF(NOW(),created_on)) minutes FROM caller_verify WHERE status = 0 AND number = '".$caller_number."' order by created_on desc limit 1";
		$rs            = $this->db->query($sql);
		...
		if($caller_number)
		{
			...
			if($rs->num_rows() > 0)
			{
				...
				else
				{
					...
					if($appointment_id)
					{
						$this->db->query("update appointment set is_verified=1 where id = '".$appointment_id."'");
					}
