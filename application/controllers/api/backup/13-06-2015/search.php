	...
	public function doctorlist_by_speciality_location_post()
	{
		...
		if(is_array($doctors) && sizeof($doctors)>0)
		{
			...
			foreach($doctors as $dKey=>$dVal)
			{
				...
				{
					$doctors[$dKey]['clinic_location_name']	=	$dVal['clinic_other_location'];
				}
				else
				{
				...
				}else if(!empty($doctors[$dKey]['clinic_latitude']) && !empty($doctors[$dKey]['clinic_longitude']) && isset($location_array['latitude']) && isset($location_array['longitude']))
				{
				...
				else if(isset($location_array['latitude']) && isset($location_array['longitude']) && !empty($device_latitude) && !empty($device_longitude)  )
				{
				
					$doctors[$dKey]['distance'] = number_format($this->location_model->distance_by_lat_lng($location_array['latitude'],$location_array['longitude'],$device_latitude,$device_longitude,"K"),0);
				}
