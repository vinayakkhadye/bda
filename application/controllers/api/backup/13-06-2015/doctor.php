...
class Doctor extends REST_Controller
{
		...
		else if($clinic_id)
		{
			...
			if($clinic)
			{
				...
				if($clinic['location_id']){
					$clinic['location_name'] = $this->common_model->getLocation(array('limit' =>1,'status'=>1,'column'=>array('id','name'),'id'    =>$clinic['location_id']));
					$clinic['location_name'] = current($clinic['location_name']);
					$clinic['location_name'] = ucwords($clinic['location_name']['name']);
	...
	function getadvanceprofile_post()
	{
		...
		if($doctor_id){
			$rs = array();
			$advance_data = $this->doctor_model->getDoctorDetail(array('doctor_id'=>$doctor_id,'limit'    =>1000));
			if($advance_data){
			...
			$rs = array("doctor_data"=>$rs,"message"    =>"succefull","status"     =>1);
		}
		else
		{
