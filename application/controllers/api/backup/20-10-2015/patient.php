	...
	function save_appointment_post()
	{
		...
		else
		{
			...
			if($patient_id==false)
			{
				...
				array('email'=>$patient_email,'name'=>$patient_name,'mobile_number'=>$mobile_number,'gender'=>$patient_gender));
		...
		if($appointment_id == "")
		{
			...
			if($appointment_id)
			{
				...
				$caller_number = '+91'.$mobile_number;
				$rs            = $this->db->query("INSERT INTO `caller_verify`(`number`,`appointment_id`)VALUES ('".$caller_number."','".$appointment_id."')");
