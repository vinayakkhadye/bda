...
class User extends REST_Controller
{
	...
	function signup_post()
	{
		...
		if($user_id)
		{
			...
			{
				$sql	=	"select id from patient where email='".$data['email'] ."' limit 1";
				$patient_query	=	$this->db->query($sql);
				if($patient_query->num_rows()>0)
				{
					#$patient_id_data	=	$patient_query->row_array();	
					$patient_id				=	$patient_id_data['id'];
					#$this->patient_model->update_patient_userid_by_emailid($data["email"],$user_id);
				}
				else
				{
					$patient_data = array(
		...
		else if($user_id && $type == 1)
		{
			...
			if(is_array($user_id_arr) && sizeof($user_id_arr) > 0)
			{
				foreach($user_id_arr as $user_val)
				{
					...
					$sql       = "delete from package_registration where user_id = ".$user_val;
					$this->db->query($sql);
					#$sql       = "delete from patient where user_id = ".$user_val;
					#$this->db->query($sql);
