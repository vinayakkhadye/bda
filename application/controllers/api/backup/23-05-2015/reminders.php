...
class Reminders extends REST_Controller
{
	...
	function get_user_packages_post()
	{
		...
		else
		{
			...
			$rs = array("message"=>$this->db->last_query(),"status"=>0);
		}
		$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code
	}
	...
	function running_late_post()
	{
		...
		if(empty($doctor_id) || $is_from_date==false || $is_to_date==false || empty($this->user_id) || empty($this->communication_id) || empty($this->communication_text) 
		|| empty($doctor_name) || empty($time))
		{
			$rs = array("message"=>"please provide user_id, doctor_id and start_date_time, end_date_time, communication_text, doctor_name, time","status" =>0);
		...
		if(is_array($running_late_appointments) && sizeof($running_late_appointments)>0)
		{
			...
			if($communication_trans_id)
			{
				...
				$this->sms_package_model->delete_communication_for_user($this->communication_name, $this->communication_type, $this->user_id,NULL,NULL);
				$rs	=	array("data"=>$sms_responce,"status" =>1);
			}
			else
			{
	...
	function get_festival_list_post()
	{
		...
		$comn_temp	=	$this->sms_package_model->get_template_by_communication('6','sms');
		$comm_data	=	$this->get_communication_of_user();
		#print_r($communication_data);exit;	
		#print_r($rs);
		#print_r($comn_temp);exit;
		if($rs)
		{
			...
			{
				$communication_data[$val->occasion_name]	=	$val->communication_text;
			}
			...
			foreach($rs as $key=>$val)
			{
				...
				$val->status	=	0;
				if(isset($communication_data[$val->name]))
				{
					$val->status	=	1;
				}
	...
	function add_communication_for_user()
	{
		...
		if($communication_transaction)
		{
			...
			{
				$communication_affected_rows	=	$this->sms_package_model->active_communication_for_user($communication_trans_id,$communication_text,$health_checkup_days,$occasion_name,$occasion_date);	
			}
		}
		else
		{
