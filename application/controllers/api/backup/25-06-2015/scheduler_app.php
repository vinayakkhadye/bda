...
class Scheduler_app extends REST_Controller
{
	...
	function GetAllPatientsByDoctorId_post()
	{
		...
		$patient_data 	= $this->patient_model->get_patient_list(array('doctor_id'=>$doctor_id,'limit'=>$limit,'offset'=>$offset,'patient_name'=>$patient_name,'patient_email'=>$patient_email));
		if($t)
		{
			echo $this->patient_model;exit;
		}
		$patient_count 	= $this->patient_model->get_patient_list_count(array('doctor_id'=>$doctor_id,'patient_name'=>$patient_name,'patient_email'=>$patient_email));
	...
	function EditPatient_post()
	{
			...
			if(empty($patient_id)|| empty($patient_email) ||empty($patient_name) ||empty($patient_gender) ||empty($mobile_number))
			{
				$rs = array("message"=>"please provide patient_id, patient_email, patient_name, patient_gender, patient_contact_no","status"=>0);	
				$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code 
			}
