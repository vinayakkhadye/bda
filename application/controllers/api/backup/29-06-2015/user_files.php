...
{
	private $log_file 	= ""; 
	private $max_file_size	=	1000000;#in bytes
	private $file_application_types	=	array("application/zip", "application/x-zip", "application/x-zip-compressed","application/pdf","image/jpeg","image/png");#in bytes
	...
	function list_post()
	{
		...
		if($file_list)
		{
			...
			$rs = array("file_data"=>$file_list,"status" =>1);
		}
		else
		{
				$rs = array("message"=>"no files","status" =>0);
		}
		$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code
	}
	...
	function edit_post()
	{
		...
		#$this->log_message($this->log_file,"request_params : ".json_encode($this->post()).NEW_LINE);
	...
	function fileupload($base64_file,$file_name)
	{
		...
		if(file_put_contents($filename_path, $decoded_pic))
		{
			...
			if(filesize($file) > $this->max_file_size)
			{
					...
					return array(FALSE,"File size cannot grater than 1 MB");
			}
