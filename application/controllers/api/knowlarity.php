...
class Knowlarity extends REST_Controller
{
	...
	public function getagentlist_get()
	{
		...
		if($is_work_on)
		{
			...
			$rs            = $this->db->query("SELECT id as 'agent_id',name as 'agent_name',number as 'agent_number' FROM agents WHERE STATUS=1 AND isbusy=0 order by id asc");
			if($rs->num_rows() > 0)
			{
	...
	{
		$call_id   = trim($this->get('bda_call_id')); # unique call id will refer to call_phone_record call_id
		$status 	 = trim(strtolower($this->get('status'))); ## will update in response of the call_phone_record table 

		$file_log = DOCUMENT_ROOT."logs/call_status.log";
		$this->log_file = fopen($file_log, "a+"); 
		$this->log_message($this->log_file,json_encode($this->get()).NEW_LINE);
		...
		$this->db->update('call_phone_record',$set);
		//$sql	=	"update call_phone_record set response=$status,counter=counter+1 where call_id = '".$call_id."' and counter<3 and response=0";
		//$this->db->query($sql);
		if($this->db->affected_rows()>0)
		{
			$rs = array("message"=>"call status updated successfully","status" =>1,"query"=>$this->db->last_query());
		}
		else
		{
			$rs = array("message"=>"call status not updated","status" =>0,"query"=>$this->db->last_query());
		}
