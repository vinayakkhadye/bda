...
class Reminders extends REST_Controller
{
	...
	function running_late_post()
	{
		...
		if(is_array($running_late_appointments) && sizeof($running_late_appointments)>0)
		{
			...
			if($communication_trans_id)
			{
				...
				if($blanace_sms==0)
				{
					$rs	=	array("message"=>"No SMS balance available","status" =>0);
					$this->response(array("response"=>$rs), 200);
				}
				...
				foreach($running_late_appointments as $a)
				{
					...
					$sms_text	=	$this->process_communication_text($this->communication_text,array('doctor_name'=>$doctor_name,'time'=>$time));
					...
					$this->communication_run_type, $response);
					for($sms_i=1;$sms_i<=$sms_cont;$sms_i++)
					{
						...
						if($blanace_sms==0)
						{
							...
							$rs	=	array("data"=>$sms_responce,"status" =>0,"message"=>"You have run out of SMS balance. Buy SMS package");
			...
			{
					$rs	=	array("message"=>"Oops!! something went wrong. Please try again","status" =>0);
			}
		}
		else
		{
			$rs	=	array("message"=>"No appointments booked in selected time period.","status" =>0);
		}
		$this->response(array("response"=>$rs), 200);
	}
	...
	function cancel_appointments_post()
	{
		...
		|| empty($doctor_name))
		{
			$rs = array("message"=>"Please provide cancellation date, time and message","status" =>0);
			$this->response(array("response"=>$rs), 200);
		}
		...
		if(!empty($cancelled_appointments))
		{
			...
			if($communication_trans_id)
			{
				...
				foreach($cancelled_appointments as $a)
				{
					...
					$sms_text	=	$this->process_communication_text($this->communication_text,
																								array('doctor_name'=>$doctor_name,'patient_name'=>$a['patient_name']));
					$sms_text_count	=	strlen($sms_text);
					$sms_cont	=	ceil($sms_text_count/160);
					...
					for($sms_i=1;$sms_i<=$sms_cont;$sms_i++)
					{
						...
						if($blanace_sms==0)
						{
							...
							$rs	=	array("data"=>$sms_responce,"status" =>0,"message"=>"All appointments cancelled,but sms sent to limited patients because of shortage of SMS, Buy SMS package");
	...
	{ 
		...
		{
			$rs = array("message"=>"Please select patients and message","status" =>0);
			$this->response(array("response"=>$rs), 200);
		}
		...
		if($communication_trans_id)
		{
			...
			if(isset($patient_mobile_array) && is_array($patient_mobile_array) && sizeof($patient_mobile_array)>0)
			{
				foreach($patient_mobile_array as $key=>$val)
				{
					...
					if(is_array($val_array) && sizeof($val_array)==3)
					{
						...
						{
							$sms_text	=	$this->process_communication_text($this->communication_text,
							array('doctor_name'=>$doctor_name,'patient_name'=>$patient_name,'time'=>$time));
							...
							$sms_log_id	=	$this->sms_package_model->sms_log($this->user_id, $patient_id, $communication_trans_id, $mobile_number, 
							$this->communication_name, $this->communication_type,$this->communication_run_type, $response_string);
			...
			{
				$this->sms_package_model->delete_communication_for_user($this->communication_name, $this->communication_type, $this->user_id,NULL,NULL);
				$rs	=	array("status" =>1,"message" =>"No patients selected");
			}	
		}
		else
		{
	...
	function happy_birthday_post()
	{
		...
		case 'get_communication':
			...
			{
				$rs = array("message"=>"Birthday Greetings is not switched on","status" =>0);
			}
			$this->response(array("response"=>$rs), 200);
		break;
		...
		case 'add_communication':
			...
			{
				$rs = array("message"=>"Please select a message","status"=>0);
				$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code	
			}
			...
			if($rs)
			{
				$rs = array("data"=>array("id"=>$rs),"status" =>1,"message"=>"Successful");
			}
			else
			{
				$rs = array("message"=>"Oops!! something went wrong. Please try again","status" =>0);
	...
	function general_health_check_up_post()
	{
		...
		case 'get_communication':
			...
			{
				$rs = array("message"=>"General Health Checkup is not switched on","status" =>0);
			}
			$this->response(array("response"=>$rs), 200);
		break;
		...
		case 'add_communication':
			...
			{
				$rs = array("message"=>"Please select message and number of days","status"=>0);
				$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code	
			}
			$this->communication_text	=	$this->process_communication_text($this->communication_text,array('doctor_name'=>$doctor_name));
	...
	function occasions_post()
	{
		...
		case 'get_communication':
			...
			{
				$rs = array("message"=>"Greetings notification is not switched on","status" =>0);
			}
			$this->response(array("response"=>$rs), 200);
		break;
		...
		case 'add_communication':
			...
			{
				$rs = array("message"=>"Please select message and day of occasion","status"=>0);
				$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code	
			}
	...
	{
		$sms_text	=	trim($communication_text);
		...
		if(isset($a['time']) && !empty($a['time']))
		{
			$sms_text	=	str_replace("{time}",$a['time'],$sms_text);
		}
		if(isset($a['health_checkup_days']) && !empty($a['health_checkup_days']))
		{
			$sms_text	=	str_replace("{health_checkup_days}",$a['health_checkup_days'],$sms_text);
		}
		if(isset($a['doctor_name']) && !empty($a['doctor_name']))
		{
			$sms_text	.=	" - Dr. ".trim(str_ireplace("dr.","",$a['doctor_name']));
		}
		return $sms_text;
	}
