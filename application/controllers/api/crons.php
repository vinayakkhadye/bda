				...
				foreach($communication_data as $data)
				{
					if($data->communication_name	==	'birthday')
					{
						...
						if($this->is_doctor_sms_package_valid($data->user_id))
						{
							...
							$sql					=	"SELECT p.id,p.name,p.mobile_number,p.dob 
																					FROM (`patient` AS p) 
																					...
																					AND `p`.`status` =  1 and DATE_FORMAT(p.dob,'%m-%d')	=	'".date("m-d",strtotime("now"))."'
																					GROUP BY `p`.`id`";
							...
							if(is_array($patient_list) && sizeof($patient_list)>0)
							{
								...
								if($blanace_sms	>=	$patient_list_count)
								{
									foreach($patient_list as $patient)
									{
										...
										$sms_text	=	str_replace("{clinic_name}","",$sms_text);
...
WHERE appointment.`status` = 1 AND appointment.`confirmation` = 1 AND appointment.`scheduled_time` BETWEEN '".$start."' AND '".$end."'";
	...
	function send_appointment_delay_sms()
	{
		$sql='SELECT added_on,patient_name,doctor.name doctor,CASE WHEN confirmation_date IS NULL THEN TIMESTAMPDIFF(MINUTE, added_on, NOW()) ELSE TIMESTAMPDIFF(MINUTE,added_on,confirmation_date) END AS time_minutes,appointment.status,appointment.confirmation,appointment.notes FROM appointment JOIN doctor ON appointment.doctor_id = doctor.id WHERE appointment.status < 2 AND ((DATE(added_on) = DATE(NOW()) AND TIME(added_on) BETWEEN "09:00:00" AND "20:00:00" AND DAYOFWEEK(added_on) > 1 AND ((confirmation_date IS NOT NULL AND TIMESTAMPDIFF(MINUTE,added_on,confirmation_date) > 30) OR (confirmation_date IS NULL AND TIMESTAMPDIFF(MINUTE, added_on, NOW()) > 30))) OR (DATE(added_on) = DATE(NOW()) AND TIME(added_on) < "09:00:00" AND ((confirmation_date IS NOT NULL AND TIMESTAMPDIFF(MINUTE,added_on,confirmation_date) > TIMESTAMPDIFF(MINUTE,added_on,CONCAT(DATE(NOW()), " ", "09:00:00")) + 45) OR (confirmation_date IS NULL AND TIMESTAMPDIFF(MINUTE, added_on, NOW()) > TIMESTAMPDIFF(MINUTE,added_on,CONCAT(DATE(NOW()), " ", "09:00:00")) + 45)))) ORDER BY added_on';
		...
		if($list)
		{
			...
			{
				$obj['patient_name'][$key]	=	$val['patient_name'];
				$obj['doctor'][$key]				=	$val['doctor'];
				$obj['time_minutes'][$key]	=	$val['time_minutes'];
				$obj['notes'][$key]					=	$val['notes'];
				
				if($val['status']==0)
				{
					$obj['status'][$key]	=	'cancelled';
				}
				else if($val['confirmation']==0)
				{
					$obj['status'][$key]	=	'pending';
				}
				else if($val['confirmation']==1)
				{
					$obj['status'][$key]	=	'confirmed';
				}
				else if($val['confirmation']==2)
				{
					$obj['status'][$key]	=	'in process';
				}
				else if($val['confirmation']==3)
				{
					$obj['status'][$key]	=	'duplicate';
				}
			}
			$mail_res = $this->mail_model->appointment_delay("datateam@bookdrappointment.com","Preethika Hegde",$obj);
			$mail_res = $this->mail_model->appointment_delay("roshni@bookdrappointment.com","Roshni Attavar",$obj);
			$mail_res = $this->mail_model->appointment_delay("sachin@bookdrappointment.com","Sachin Mishra",$obj);
			//$mail_res = $this->mail_model->appointment_delay("dipen@bookdrappointment.com","Preethika Hegde",$obj);
			$mail_res = $this->mail_model->appointment_delay("tjpoojary44@gmail.com","Tj Poojary",$obj);
		}				
	}
	
	function call_phone_appointments()
	{
		$this->log_file = fopen(DOCUMENT_ROOT."logs/call_phone_appointments.log", "a+");			
		$sql	=	"select id,caller_number,agent_number,call_id,extension FROM call_phone_record WHERE response=0 AND counter<3 and call_status='Connected' ORDER BY id ASC";
		$query	=	$this->db->query($sql);
		$list = $query->result_array();
		...
		if($list)
		{
			...
			foreach($list as $key=>$val)
			{
				if($val['agent_number']=='912249426426')
				{
						$sql	=	"update call_phone_record set response=1,counter=1 where id='".$val['id']."'";
						$query	=	$this->db->query($sql);
				}
				else
				{	
					$agent_number		=	substr($val['agent_number'],-8,strlen($val['agent_number']));
					$sql	=	"select doctor_id from clinic where contact_number like	'%".$agent_number."%' and status=1 and knowlarity_extension	=	".$val['extension']." limit 1";
					$query	=	$this->db->query($sql);
					$clinic = $query->row_array();
					
					if($clinic && is_array($clinic))
					{
						$sql	=	"select name from doctor where id	=	".$clinic['doctor_id']." and status=1 limit 1";
						$query	=	$this->db->query($sql);
						$doctor = $query->row_array();
						
						if($doctor && is_array($doctor))
						{
							$knowlarity_obd_api	=	"http://etsdom.kapps.in/webapi/bda/api/bda_obd.py?customer_number=".$val['caller_number']."&doctor_name=".$doctor['name']."&doctor_id=".$clinic['doctor_id']."&auth_key=6842c1ee-ac05-4e90-8b6f-0d40e40240d4&bda_call_id=".$val['call_id'];
							$this->log_message($this->log_file,"API : ".$knowlarity_obd_api.NEW_LINE);
							$this->log_message($this->log_file,"API : ".NEW_LINE);
							file_get_contents($knowlarity_obd_api);
						}
					}
					else
					{
						$this->log_message($this->log_file,"not found clinic number : ".$val['agent_number'].", "." extension : ".$val['extension'].NEW_LINE);
						$this->log_message($this->log_file,"API : ".NEW_LINE);
					}
				}
			}
			$this->log_message($this->log_file,"------------- END ------------------".NEW_LINE);
		}
	}
}