...
class favourite_doctor extends REST_Controller
{
	...
	function add_post()
	{
		...
		{
			$rs = array("message"=>"Oops!! Not added to favourites","status" =>0,"error"=>$this->db->_error_message());
		}
		$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code
	}
	...
	function delete_post()
	{
		...
		{
			$rs = array("message"=>"Oops!! Not deleted from favourite","status" =>0,"error"=>$this->db->_error_message());
		}
		$this->response(array("response"=>$rs), 200); // 200 being the HTTP response code
	}
