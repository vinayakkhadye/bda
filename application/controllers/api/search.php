...
class Search extends REST_Controller
{
	...
	public function doctor_web_get($city_name){
		$city	 = $city_name; 
		$query =  urldecode($this->get('query')); 
		...
		sort($retArray);
		$new_arr	=	 array();
		foreach($retArray as $ret_key=>$ret_val)
		{
			$new_arr[$ret_key]['data']	=	$ret_key;
			$new_arr[$ret_key]['value']	=	$ret_val;
		}
		
		echo  json_encode(array('suggestions'=>$new_arr));
		return false;
	}
	...
	public function clinic_web_get($city_name){
		$this->load->model(array('common_model'));
		$city = $city_name;
  ...
  public function phone_number_post()
	{
		...
		$no_doctor_details = $this->post('no_doctor_details');
		...
		$sql	=	"select contact_number,knowlarity_number,knowlarity_extension from clinic where id=".$clinic_id;
		$query =	$this->db->query($sql);
		if($query->num_rows > 0)
		{
			...
			{
				$rs = array(
				"number_data"=>$data[0]['contact_number'],
				"knowlarity"=>array("number"=>$data[0]['knowlarity_number'],"extension"=>$data[0]['knowlarity_extension']),
